...
const Navbar = () => {
  ...
  return (
    <nav className='bg-zinc-950 text-white p-4 shadow-lg sticky top-0 z-50'>
        ...
        <div className='hidden md:flex text-lg items-center gap-8'>
          <NavLink to='/create' className={({ isActive }) =>
            `flex items-center gap-1 font-semibold transition-all ${
              isActive ? 'text-amber-500' : 'text-zinc-500 hover:text-amber-500'
            }`
          }>Create<MdAddBox /></NavLink>

          <NavLink to='/profile' className={({ isActive }) =>
            `flex items-center gap-1 font-semibold transition-all ${
              isActive ? 'text-amber-500' : 'text-zinc-500 hover:text-amber-500'
            }`
          }>Profile<MdPerson /></NavLink>

          <NavLink to='/dashboard' className={({ isActive }) =>
            `flex items-center gap-1 font-semibold transition-all ${
              isActive ? 'text-amber-500' : 'text-zinc-500 hover:text-amber-500'
            }`
          }>Dashboard<MdDashboard /></NavLink>
        </div>
      ...
      >
        <NavLink to='/create' onClick={() => setIsOpen(false)} className={({ isActive }) =>
          `text-lg flex items-center gap-1 font-semibold transition-all ${
            isActive ? 'text-amber-500' : 'text-zinc-300 hover:text-amber-500'
          }`
        }>Create<MdAddBox /></NavLink>

        <NavLink to='/profile' onClick={() => setIsOpen(false)} className={({ isActive }) =>
          `text-lg flex items-center gap-1 font-semibold transition-all ${
            isActive ? 'text-amber-500' : 'text-zinc-300 hover:text-amber-500'
          }`
        }>Profile<MdPerson /></NavLink>
